const validator = {
  path: {
    source: [(v) => !!v || '역을 선택해주세요.', (v) => v < 1 || '유효하지 않은 역입니다.'],
    target: [(v) => !!v || '역을 선택해주세요.', (v) => v < 1 || '유효하지 않은 역입니다.', (v) => v === this.source || '출발역과 다른 역을 선택해주세요.'],
  },
  departureTime: {
    dayTime: [(v) => v === '오전' || v === '오후' || '오전 혹은 오후를 선택해주세요.'],
    hour: [(v) => v >= 1 || v <= 12 || '시간은 1과 12사이어야 합니다.'],
    minute: [(v) => v >= 0 || v <= 60 || '분은 0과 60 사이어야 합니다.']
  },
